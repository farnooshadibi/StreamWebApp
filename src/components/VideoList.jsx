    ...
    render(){
        ...
        return(
            <div className="col-lg-4 col-md-6 d-flex justify-content-around" style={{marginBottom:10}}>
                ...
                <img src={customer.image} className="card-img-top" alt={customer.name} style={{width:"auto" , height:'18rem' }}/>
                    <div className="card-body">
                        <h5 style={{fontSize:"25px"}} className="card-title">{customer.name}</h5>
                        {/* <p className="card-text">{customer.name.substr(0 , 100)} ...</p> */}
                    </div>
                    <div style={{backgroundColor:"#000", width:"100%", height:"20%"}}>
                        <Link className="btn btn-success" style={{ textAlign:'center' }} to={`video-detail/${customer.id}`}> مشاهده</Link>
                    </div>
                </div>
            </div>
            </div>
